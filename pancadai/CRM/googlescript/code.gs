/* Code.gs - PancadAI CRM Backend V4.9 (Add KOL Phone) */
...
function doPost(e) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'saveKOL': result = saveKOL(data.payload, userEmail); break; // 更新了此函式
...
function saveKOL(p, u) { 
    ...
    if(p.kolId) { 
        for(let i=1; i<data.length; i++) {
            if(String(data[i][0]) === String(p.kolId)) {
                r = i + 1;
                break;
            }
        } 
    ...
    // 資料欄位對應: [ID, Name, Hospital, Title, Phone, Email, Stage, Note, Probability, ..., ..., Date]
    // 注意：p.phone 對應到第 5 欄 (Index 4)
    const row = [
        id, 
        p.name, 
        p.hospitalId, 
    ...
    if(r > 0) {
        ...
        logAction(u, 'Update KOL', `${p.name}`);
    } else {
        sheet.appendRow(row);
        logAction(u, 'New KOL', `${p.name}`);
    }
    return {id: id}; 
}
...
function checkUserAuth(email) { 
  ...
  const user = users.find(u => String(u.Email).toLowerCase() === String(email).toLowerCase() && u.Status === 'Active'); 
