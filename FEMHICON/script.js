...
// === 3. 初始化 Slot State ===
let slotState = [
    { type: "internal", val: 0, customText: originalIcons[0].name, style: "white" },
    { type: "internal", val: 1, customText: originalIcons[1].name, style: "white" },
    { type: "internal", val: 2, customText: originalIcons[2].name, style: "white" },
    { type: "internal", val: 3, customText: originalIcons[3].name, style: "white" },
    { type: "internal", val: 4, customText: originalIcons[4].name, style: "green" }, 
    { type: "internal", val: 5, customText: originalIcons[5].name, style: "white" }
];
...
function changeIconSource(source) {
    ...
    if (source === "internal") {
        ...
        // [關鍵修正] 選擇不顯示時，立即清空並更新狀態
        manualInputContainer.classList.remove("active");
        iconLibrary.innerHTML = "<div style='width:100%;text-align:center;padding:20px;color:#888;'>已設定為無圖示</div>";
        
        slotState[selectedSlotIndex].type = "none";
        slotState[selectedSlotIndex].val = "";
        renderCanvas();
