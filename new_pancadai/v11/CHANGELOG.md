...
- 不新增未驗證的醫療宣稱、Product／Offer schema 或非標準「AI ranking」標記；保留可見內容、canonical URL、robots 與既有結構化資料的一致性。
...
## v11.2.73（2026-09-11）— 主視覺 4K 超解析更新

### 🎨 圖片品質
- 以本機 SeedVR2 diffusion super-resolution 重新處理 18 張剛生成的中／英／日主視覺與患者頁 hero 圖。
- 所有新版 `_4k.webp` 均為 3840×2160（4K UHD），保留原有右側人物構圖與左側文字留白；以 WebP quality 92 輸出，供快速網路桌面環境的漸進載入使用。
- 低解析度、HD 與 4K 檔名及載入順序維持不變，2G／省流量與行動裝置仍優先使用較小檔案。
...
## v11.2.72（2026-09-11）— 篩檢數據與自適應 4K 圖片載入

### 🔧 內容與效能
- `/screening.html`「累積判讀人次」由 `300+` 更新為 `2000+`；英文與日文版本維持既有 2,000+ 口徑。
- 背景池、患者 hero、晚餐情境與固定 CT 圖改為低解析度先顯示，再依網路狀態逐級載入中尺寸、HD 與 4K UHD（3840×2160）版本。
- 2G／省流量模式只保留低解析度；3G 載入 HD；4G／Wi-Fi 在桌面閒置時再載入 4K，任一高解析度檔案失敗會保留已顯示的低解析度版本。
- 新增 32 個本機 4K WebP 資產，並將全站 `main.js?v=112` 快取版本更新，避免瀏覽器沿用舊載入策略。

### ✅ 驗證
- `node --check js/main.js`、`git diff --check` 通過；screening DOM 確認顯示 `2000+`。
- 本機瀏覽器讀回確認背景先載低圖，快速網路桌面環境後續升級至 `_4k.webp`。
...
### 🔧 文案與語系調整
- 英文頁面與 EN i18n 字典改以美國醫療網站常用語氣：統一使用 AI-assisted review、radiologist review、under 2 cm、multicenter 等表述，並移除世界第一、保證式偵測、AI 守護與過度療效暗示。
- 英文產品、臨床證據、健檢導入、衛教、病患旅程、出版品、法規、專利、新聞與公司介紹頁同步修正靜態備援文字、metadata、Open Graph、JSON-LD 與英文摘要。
- 英文頁面的靜態 fallback 與 i18n 字典逐一對齊，並修正 Editorial、Privacy、Terms 頁載入錯誤語系字典的問題。
- FDA Breakthrough Device Designation 與 510(k) 狀態明確標示：2026 年 7 月已送件、目前審查中，尚未取得美國上市 clearance；保留台灣 TFDA 許可證與可查證研究數據。
- 「健康台灣深耕計畫」僅保留中文版頁面與 `/deep-plan/`；英文 `/en/` 與日文 `/jp/` 頁面移除所有區塊及連結，日文共用字典鍵也清空以避免後續共用元件誤顯示。
- 內容字典同步更新快取版本參數，讓部署後的瀏覽器載入最新英文文案與語系設定。

### ✅ 本機驗證
- 全部 `js/*.js` 通過 `node --check`。
- `git diff --check` 通過；英文與日文 HTML 可由 BeautifulSoup 解析。
- `/en/`、`/jp/` HTML 搜尋 `deep-plan`、`深耕計畫`、`健康台灣` 均無命中；中文版內容與深耕入口保留，僅同步快取版本參數。
- 本輪只完成本機修改與驗證，尚未部署。

## v11.2.68（2026-09-10）— 英文產品頁臨床主視覺更新

### 🎨 視覺調整
- 英文 `product.html` 首屏移除 CT 掃描機背景，改用擬真臨床影像判讀室場景。
- 右側醫療工作站螢幕呈現腹部 CT 軸位影像，左側保留低細節深色留白，讓英文 hero 文字方框不遮住醫師與螢幕主視覺。
- 新增 `assets/product_hero_clinical.jpg`（1280×720）與 `assets/product_hero_clinical_hd.webp`（1672×941）；原始生成檔保留於 `assets/_gen_v11/`。
- 英文 hero 桌面版改以 `background-position: 75% center`、文字方框縮至 780px；手機版回到中央裁切，保留閱讀性與人物構圖。
- 全部英文頁同步升級 `en-us.css?v=7`，避免快取舊版構圖規則。

### ✅ 驗證
- 已以桌面／手機視口檢查背景裁切、文字方框、人物與腹部 CT 螢幕位置。
- GitHub commit `f5827fcf`、Deploy `34438700183`、Pages build `34438745618`；Cloudflare deployment `f6c8cba3` 均已成功。
...
### 🔧 行為調整
- 首次進入根首頁時讀取瀏覽器／OS 的主要語言：日文（`ja`）導向 `/jp/`；英文及其他語言導向 `/en/`。
- 使用者手動選擇語言後以 localStorage 記住選擇；選擇繁中會以 `?lang=zh` 回到根首頁並略過自動導向。
- 英文入口保留 query string 與 hash，導向產品首頁時不遺失來源參數。
...
### 🔧 介面調整
- 在全站桌面版語言下拉按鈕左側加入地球圖示，讓語言切換入口更容易辨識。
- 圖示採裝飾性 CSS pseudo-element，不改變語言按鈕的文字、鍵盤操作或 i18n 路由邏輯。
- 全站 47 個載入共用 `style.css` 的 HTML 頁同步升版 query string，避免瀏覽器快取舊樣式。

### ✅ 驗證
- `git diff --check` 通過。
- 中文、英文、日文頁均保留既有語言下拉選單與 `/en/`、`/jp/` 導向行為。
- 本輪僅完成本機程式變更，尚未部署。
